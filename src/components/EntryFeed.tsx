...
export default function EntryFeed({
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      {/* Horizontal scrubber */}
      <div className="shrink-0 bg-white dark:bg-surface border-b border-slate-100 dark:border-slate-700">
        <div
          ref={scrubberRef}
            ...
            return (
              <button
                ...
                className={`flex-shrink-0 px-3 py-2 rounded-xl text-xs font-semibold transition-all min-h-[48px] ${
                  isActive
                    ? "bg-slate-900 dark:bg-brand text-white dark:text-white shadow-sm"
                    : "bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-600"
                }`}
              >
        ...
        {visibleGroups.length === 0 ? (
          ...
          visibleGroups.map((group) => (
            <div key={`${group.shiftDate}-${group.shiftType}`} className="mb-6 last:mb-0">
              {/* Shift group header */}
              <div className="sticky top-0 z-10 bg-slate-50/95 dark:bg-slate-800/95 backdrop-blur-sm py-2 mb-3 -mx-5 px-5 border-b border-slate-200 dark:border-slate-700">
              ...
              <div className="relative">
                <div className="absolute left-[11px] top-4 bottom-4 w-px bg-slate-200 dark:bg-slate-700" />
                      ...
                      return (
                        <div key={entry._id} className="relative flex gap-3 group">
                          ...
                          <div className="relative z-10 w-6 flex justify-center shrink-0 pt-2">
                            <div className={`w-2 h-2 rounded-full ring-4 ring-slate-50 dark:ring-slate-800 ${classes.dot}`} />
                          </div>

                          {/* Entry Card */}
                          <div className={`flex-1 bg-white dark:bg-card-new border border-slate-100 dark:border-slate-700 rounded-2xl p-3 shadow-sm relative group-hover:border-slate-200 dark:group-hover:border-slate-600 transition-colors border-l-4 ${classes.border}`}>
                            ...
                            <button
                              ...
                              className="absolute right-2 bottom-2 p-1.5 text-slate-300 dark:text-slate-600 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition opacity-0 group-hover:opacity-100 md:opacity-0"
